...
describe("getNodeTypeOffset", () => {
  ...
  it("returns offsets for list types", () => {
    expect(getNodeTypeOffset("bulletList")).toEqual({ top: 1, left: 12 });
    expect(getNodeTypeOffset("orderedList")).toEqual({ top: 1, left: 12 });
    expect(getNodeTypeOffset("taskList")).toEqual({ top: 1, left: 12 });
    expect(getNodeTypeOffset("listItem")).toEqual({ top: 1, left: 12 });
    expect(getNodeTypeOffset("taskItem")).toEqual({ top: 1, left: 12 });
  });

  it("returns offsets for block nodes", () => {
    expect(getNodeTypeOffset("blockquote")).toEqual({ top: 2, left: 0 });
    expect(getNodeTypeOffset("codeBlock")).toEqual({ top: 2, left: 0 });
    expect(getNodeTypeOffset("table")).toEqual({ top: 2, left: 0 });
  });

  it("returns default offsets for unknown nodes", () => {
    expect(getNodeTypeOffset("paragraph")).toEqual({ top: 0, left: 0 });
    expect(getNodeTypeOffset("unknown")).toEqual({ top: 0, left: 0 });
  });
});
